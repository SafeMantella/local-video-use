...
7. **Pad every cut edge.** Working window: 30–200ms. whisper.cpp's DTW timestamps drift 50–100ms — padding absorbs the drift. Tighter for fast-paced, looser for cinematic.
...
The skill lives in `local-video-use/`. User footage lives wherever they put it. All session outputs go into `<videos_dir>/edit/`.
...
└── edit/
    ...
    ├── transcripts/<name>.json  ← cached raw whisper.cpp JSON (word-level)
...
First-time install lives in `install.md` (clone, deps, ffmpeg, skill registration, whisper.cpp build). Don't re-run it every session; on cold start just verify:

- `WHISPER_CPP_BIN` and `WHISPER_CPP_MODEL` resolve — either in the environment or in `.env` at the local-video-use repo root — and both paths exist on disk. If missing, run `install.md`'s whisper.cpp setup.
- `ffmpeg` + `ffprobe` on PATH.
- Python deps installed (`uv sync` or `pip install -e .` inside the repo).
- Node.js + npm available if the session needs HyperFrames or Remotion slots. HyperFrames currently requires Node.js 22+.
- `yt-dlp`, HyperFrames, Remotion, Manim installed only on first use.
- First-use animation setup happens inside the slot directory, never at the local-video-use repo root. HyperFrames can be invoked with `npx --yes hyperframes ...`; Remotion can be scaffolded with `npx create-video@latest` or installed as a project-local dependency before using its `remotion render` command.
- This skill vendors `skills/manim-video/`. Read its SKILL.md when building a Manim slot.

Helpers (`helpers/transcribe.py`, `helpers/render.py`, etc.) live alongside this SKILL.md. Resolve their paths relative to the directory containing this file — the skill is typically symlinked at `~/.claude/skills/local-video-use/` or `~/.codex/skills/local-video-use/`.

## Helpers

- **`transcribe.py <video>`** — single-file local whisper.cpp transcription (Metal-accelerated). `--diarize --num-speakers N` optional, for multi-speaker takes. Cached.
...
- **SRT / phrase-level transcript output.** Loses sub-second gap data. Always word-level verbatim — `transcribe.py` reconstructs word timestamps from whisper.cpp's DTW-aligned token stream (`-ojf -dtw`), never from `-osrt`.
- **Running whisper.cpp without `-dtw` / without Metal.** Without DTW alignment, per-token timestamps bunch up mid-segment (many words share one timestamp) — always pass `-dtw <preset>` (see `transcribe.py`). CPU-only inference is also much slower than Metal; confirm the build log said "Including METAL backend" during install. Whisper models still normalize fillers/hesitations somewhat more than Scribe did — accepted trade-off for zero API cost.
